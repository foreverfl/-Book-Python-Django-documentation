...
class IndexView(generic.ListView):
    ...
    def get_queryset(self): # 모델을 지정함
        return Question.objects.filter(pub_date__lte=timezone.now()).order_by("-pub_date")[:] # pub_date 필드의 값이 timezone.now()보다 작거나 같은 객체만 선택
...
class DetailView(generic.DetailView):
    ...
    def get_queryset(self):
        return Question.objects.filter(pub_date__lte=timezone.now()) # 과거의 질문만 보여주도록 함
